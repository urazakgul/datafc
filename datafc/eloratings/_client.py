...
class EloRatingsClient:
    """
    HTTP client for the eloratings.net public site. Responses are TSV text.

    Cached payloads are stored as ``{"tsv": "<text>"}`` so they share the same
    DiskCache backend used by the Sofascore and ClubElo clients.
    ...
    def _rate_limit_wait(self) -> None:
        with EloRatingsClient._class_lock:
            elapsed = time.monotonic() - EloRatingsClient._class_last_request_time
            if elapsed < self._min_interval:
                time.sleep(self._min_interval - elapsed)
            EloRatingsClient._class_last_request_time = time.monotonic()

    def get(self, url: str) -> str:
        """Perform a GET, return the response body as TSV text."""
        if self._cache is not None:
            cached = self._cache.get(url)
            if isinstance(cached, dict) and "tsv" in cached:
    ...
    def __enter__(self) -> "EloRatingsClient":
        return self
