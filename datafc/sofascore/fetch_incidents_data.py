...
def incidents_data(
    ...
    Returns one row per incident. Columns common to all types: ``incident_type``,
    ``incident_class``, ``time``, ``added_time``, ``is_home``, ``player_id``,
    ``player_name``. Type-specific columns (``home_score``/``away_score``/
    ``goal_from`` for goals; ``card_reason``/``rescinded`` for cards;
    ``var_confirmed`` for VAR decisions) are ``None`` when not applicable.
    ...
    Returns:
        Incident data with goal, card and VAR decision events per match.

    Raises:
        InvalidParameterError: If an invalid data_source is given or match_df is empty.
        DataNotAvailableError: If no incidents are returned.
    ...
    with SofascoreClient(rate_limit=rate_limit, cache=cache) as client:
        for _, row in match_df.iterrows():
            ...
            try:
                ...
                logger.warning("Failed to fetch incidents for game_id=%s: %s", game_id, exc)
                continue
            records.extend(parse_incidents_records(data, country, tournament, season, week, game_id))
