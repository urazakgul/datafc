...
def shots_data(
    ...
    Returns:
        Shot map data with xG, coordinates, shot type and player info per shot.

    Raises:
        InvalidParameterError: If an invalid data_source is given or match_df is empty.
        DataNotAvailableError: If no shot data is returned.
    ...
    with SofascoreClient(rate_limit=rate_limit, cache=cache) as client:
        for _, row in match_df.iterrows():
            ...
            try:
                data = client.get(f"{API_URLS[data_source]}/api/v1/event/{game_id}/shotmap")
            except APIError as exc:
                logger.warning("Failed to fetch shots for game_id=%s: %s", game_id, exc)
                continue
            records.extend(parse_shots_records(data, country, tournament, season, week, game_id))
