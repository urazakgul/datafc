...
def match_odds_data(
    ...
    Returns:
        Odds data per market and choice with initial and current fractional values.

    Raises:
        InvalidParameterError: If an invalid data_source is given or match_df is empty.
        DataNotAvailableError: If no odds data is returned.
    ...
    with SofascoreClient(rate_limit=rate_limit, cache=cache) as client:
        for _, row in match_df.iterrows():
            ...
            data = client.get(f"{API_URLS[data_source]}/api/v1/event/{game_id}/odds/1/all")
            records.extend(parse_match_odds_records(data, country, tournament, season, week, game_id))
