...
def average_positions_data(
    ...
    Returns each player's mean position on the pitch during the match, calculated
    from all tracked touch events. Useful for tactical and formation analysis.
    Coordinates are on a 0–100 scale for both axes.
    ...
    Returns:
        ...
        home_team, away_team, side, player_name, player_id, position,
        jersey_number, average_x, average_y, points_count.

    Raises:
        InvalidParameterError: If an invalid data_source is given or match_df is empty.
        DataNotAvailableError: If no positional data is found.
    ...
    with SofascoreClient(rate_limit=rate_limit, cache=cache) as client:
        for _, row in match_df.iterrows():
            ...
            try:
                ...
                logger.warning("Failed to fetch average positions for game_id=%s: %s", game_id, exc)
                continue
            records.extend(parse_average_positions_records(
