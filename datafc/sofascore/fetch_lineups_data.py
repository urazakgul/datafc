...
def lineups_data(
    ...
    """
    Fetches lineup and per-player statistics for each match in the provided match dataset.
    ...
    Returns:
        Player lineup data with individual statistics per player per match.

    Raises:
        InvalidParameterError: If an invalid data_source is given or match_df is empty.
        DataNotAvailableError: If no lineup data is returned.
    ...
    with SofascoreClient(rate_limit=rate_limit, cache=cache) as client:
        for _, row in match_df.iterrows():
            ...
            try:
                data = client.get(f"{API_URLS[data_source]}/api/v1/event/{game_id}/lineups")
            except APIError as exc:
                logger.warning("Failed to fetch lineups for game_id=%s: %s", game_id, exc)
                continue
            records.extend(parse_lineups_records(data, country, tournament, season, week, game_id))
