...
def substitutions_data(
    ...
    """
    Fetches substitution events for each match in the provided match dataset.
    ...
    Returns:
        Substitution events with minute, player in/out names and IDs.

    Raises:
        InvalidParameterError: If an invalid data_source is given or match_df is empty.
        DataNotAvailableError: If no substitution events are found.
    ...
    with SofascoreClient(rate_limit=rate_limit, cache=cache) as client:
        for _, row in match_df.iterrows():
            ...
            try:
                ...
                logger.warning("Failed to fetch substitutions for game_id=%s: %s", game_id, exc)
                continue
            records.extend(parse_substitutions_records(data, country, tournament, season, week, game_id))
